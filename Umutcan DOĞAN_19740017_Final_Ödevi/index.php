      ...
      <div class="col-lg-7">
        <div class="card card-outline-info">
          ...
          <?php
              ...
              //?veri kayıt
                  $query = $db->query("SELECT * FROM yemekler");
                  $yemek = $query->fetchall(PDO::FETCH_ASSOC); 
          ...
          foreach ($yemek as $yemek){//?-------------------------------------------------------------------------- klasör okunup döngüye atandı
                echo '            <div class="card">
                <div class="card-header" id="headingThree">
                  <h5 class="mb-0">
                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapse'.$x.'">     <!-- collapse için benzersiz id değişkeni -->
                      '.$yemek['yemek_adi'].'                                                                                               <!-- include ile sayfaya dahil edilen yemek adı değişkeni yazıldı -->
                    </button>
                  </h5>
                </div>
                <div id="collapse'.$x.'" class="collapse">                                                                         <!-- collapse için benzersiz id değişkeni -->
                  <div class="card-body">';
                  $malzeme = explode('|', $yemek['malzeme_listesi']);
                  foreach($malzeme as $malzeme){//?----------------------------------------------------------------------- foreach ile malzeme listesi dizisi malzeme değişkenine atandı
                      echo '<li>'.$malzeme.'</li>';//?---------------------------------------------------------------------------- malzeme değişkeni li etiketleri ile yazıldı
                  }
                   echo'<hr>
                   <p>'.$yemek['tarif'].'</p>                                                                                               <!-- include ile sayfaya dahil edilen tarif değişkeni yazıldı -->
                   </div>
                </div>
              </div>';
